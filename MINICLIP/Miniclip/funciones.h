...
void salirDelJuego(int key)
{
    key = rlutil::getkey();
    int keyop;
    if (key == rlutil::KEY_ESCAPE)
    {
        recuadro(95, 15, 50, 10, 0, 15);
        rlutil::locate(97, 20);
        std::cout << "Quieres salir del juego----> 1-SI ---- 2-NO: ";
        rlutil::locate(143, 20);
        std::cin >> keyop;
        ...
        {
            rlutil::setBackgroundColor(rlutil::COLOR(0));
